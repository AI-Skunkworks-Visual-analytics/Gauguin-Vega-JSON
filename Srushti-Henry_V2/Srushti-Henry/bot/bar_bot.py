...
class BarBot():
    ...
    def simple_bar_JSON_generator(self) -> any:
        ...
        for i in range(len(self.cat_col)):
            for j in range(len(self.num_col)):
                file_path = self._generate_plot_path( str(self.cat_col[i]), str(self.num_col[j]),  BAR.SIMPLE_BAR.value)
                if file_path == "":
                    pass
                else:
    ...
    def stacked_bar_JSON_generator(self) -> any:
        ...
        for i in range(len(self.cat_col)-1):
            file_path = self._generate_plot_path( str(self.cat_col[i]), str(self.cat_col[i+1]),  BAR.STACKED_BAR.value)
            if file_path == "" :
                pass
            else:
...
if __name__ == "__main__": 
    my_bar = BarBot( file_name = DATA_NAME_LIST[1], chart_type = 'bar' )

    my_bar.null_count( 30 )

    my_bar.unique_value_list()

    my_bar.new_folder_for_log_and_plot( BAR )
