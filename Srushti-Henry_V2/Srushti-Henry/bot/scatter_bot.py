    ...
    def scatter_JSON_generator(self) -> any:
        ...
        for i in tupls:
            ...
            if file_path == "" :
                ...
                chrt= alt.Chart(temp).mark_circle(size=60).encode(
                    x=filtered_column_name0,
                    y=filtered_column_name1)

                chrt.save(file_path)
        print("Scatter JSON generated in ""Altair_Plots"" folder for the combinations")



if __name__ == "__main__": 
    my_bar = ScatterBot( file_name = DATA_NAME_LIST[1], chart_type = 'scatter' )

    my_bar.null_count( 30 )

    my_bar.unique_value_list()

    my_bar.new_folder_for_log_and_plot( SCATTER )
